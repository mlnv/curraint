...
import { getUnclosedReasoningTagStart, hasThinkTag, parseThinkTags, stripLeadingReasoningTag } from '.';
...
describe('parseThinkTags', () => {
  ...
  it('supports multiline think blocks', () => {
    ...
    expect(parseThinkTags(content)).toEqual([{ type: 'think', content: 'line one\nline two' }]);
  });
  ...
  it('extracts escaped think blocks', () => {
    expect(parseThinkTags('A&lt;think&gt;x&lt;/think&gt;B')).toEqual([
      { type: 'text', content: 'A' },
      { type: 'think', content: 'x' },
      { type: 'text', content: 'B' }
    ]);
...
describe('hasThinkTag', () => {
  it('detects think tag blocks', () => { expect(hasThinkTag('x<think>y</think>z')).toBe(true); });
  it('returns false when no think tags', () => { expect(hasThinkTag('no tags')).toBe(false); });
  it('detects reasoning tag blocks', () => { expect(hasThinkTag('x<reasoning>y</reasoning>z')).toBe(true); });
...
describe('getUnclosedReasoningTagStart', () => {
  ...
  it('returns start index for unclosed escaped tag', () => {
    ...
    expect(getUnclosedReasoningTagStart(content)).toBe(content.indexOf('&lt;reasoning&gt;'));
  });
});

describe('stripLeadingReasoningTag', () => {
  it('removes raw opening think tag', () => { expect(stripLeadingReasoningTag('<think>abc')).toBe('abc'); });
  it('removes raw opening reasoning tag', () => { expect(stripLeadingReasoningTag('<reasoning>abc')).toBe('abc'); });
  it('removes escaped opening tag', () => { expect(stripLeadingReasoningTag('&lt;think&gt;abc')).toBe('abc'); });
});
